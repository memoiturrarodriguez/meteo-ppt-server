...
def limiting_codes(block, opname=None):
    """
    Abreviaciones de limitantes meteorológicas para el Gantt.
    Debe devolver siempre una causa visible cuando una operación queda marginal/restringida,
    evitando celdas rojas/naranjas vacías.
    Formato compacto en una línea: RV/N/P, para que no se pierdan letras en celulares.
    """
    ...
    # Viento/rachas: usar umbral propio de la operación cuando exista.
    ...
    if restrict is not None and wind_ref >= restrict:
        ...
        codes.append("RV")
    elif "VIENTO" in motivo or "RACHA" in motivo or wind_ref >= 13:
        codes.append("RV")
    ...
    # Nubosidad/techo/condición visual.
    if "NUB" in motivo or "TECHO" in motivo or "BASE DE NUBES" in motivo or nub >= 90:
        codes.append("N")

    # Criterio especial SBM: techo <3200 ft afecta seguridad de lanzamiento.
    if opname and ("SALTO BASICO" in opn or opn == "SBM") and techo is not None and 0 < techo < 3200:
        codes.append("N")

    # Otras operaciones aéreas: techo/base muy baja también se muestra como limitante visual.
    if opname and is_aerial_operation(opname) and techo is not None and 0 < techo < 1000:
        codes.append("N")

    # Temperatura: calor y frío operacional.
    if temp is not None and temp >= 25:
        codes.append("T")
    if temp is not None and temp <= 5:
        codes.append("TF")

    # Montaña/nieve/viento blanco y costero.
    if nieve > 0 and (viento >= 15 or rachas >= 20):
        codes.append("VB")
    if oleaje >= 1.5 or "OLEAJE" in motivo or "MAR" in motivo:
        codes.append("O")

    # Si la operación está restringida/marginal pero aún no hay causa visible, dejar marca genérica.
    if opname:
        est = get_op_state(block, opname)
        if estado_score(est) > 0 and not codes:
            # Intento adicional según palabras del motivo original.
            if "FRIO" in motivo or "BAJA T" in motivo or "TEMPERATURA" in motivo:
                codes.append("TF")
            elif "CAMBIO" in motivo or "METEOR" in motivo:
                codes.append("MET")
            else:
                codes.append("MET")
    ...
    # Compacto para evitar saltos verticales que se cortan en PowerPoint móvil.
    return "/".join(seen[:3])
...
def slide_gantt(prs, data):
    ...
    for r, (op, lab) in enumerate(zip(ops, labels), start=2):
        ...
        for c in range(1, cols):
            ...
            if estado_score(est) > 0:
                codes = limiting_codes(b, op)
                # Mostrar también la operación dentro de la celda para que no se pierda al visualizar/recortar en celular.
                cell_txt = f"{lab}\n{codes}" if codes else lab
                cell_size = 7.0 if codes else 7.8
            else:
                cell_txt = ""
                cell_size = 8.0
            set_cell(table.cell(r, c), cell_txt, cell_size, True, status_color(est))
    ...
    add_text(slide, "Celda = operación + limitante. RV = rachas/viento   P = precipitación   V = visibilidad\nN = nubosidad/techo   T = calor   TF = frío ≤5°C   VB = viento blanco/nieve   O = oleaje", 3.85, 6.48, 5.1, 0.35, 6.8, False, WHITE)
