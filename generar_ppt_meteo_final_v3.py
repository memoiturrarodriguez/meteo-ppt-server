...
def _aerial_forced_rank(block, opname):
    ...
    # Visibilidad: no sobre-restringir por valores aún operacionales.
    # Criterio PPT/SOW: >=5 km no marca limitante; <5 km marginal; <=1 km restringido.
    # Esto evita marcar V con 9-10 km cuando el cuadro no muestra un problema real.
    if vis is not None:
        if vis <= 1:
            rank = max(rank, 2)
        elif vis < 5:
    ...
    # Techo/base de nubes.
    # Criterio definido por el usuario para SBM y LC:
    # - <=3300 ft: RESTRINGIDO
    # - 3301 a 3499 ft: MARGINAL
    # - >=3500 ft: FAVORABLE respecto a techo
    if techo is not None:
        op_norm = normalize(opname)
        if "SALTO BASICO" in op_norm or op_norm == "SBM" or "LANZAMIENTO" in op_norm or op_norm == "LC":
            if techo > 0 and techo <= 3300:
                rank = max(rank, 2)
            elif 3300 < techo < 3500:
                rank = max(rank, 1)
        else:
...
def _non_aerial_forced_rank(block, opname):
    ...
    rank = 0
    if "MARCHA" in n:
        # Visibilidad solo limita marcha si cae bajo 5 km.
        if vis is not None and vis < 5:
            rank = max(rank, 1)
        # Temperatura alta/fría afecta MARCHA/PERNOCTA, no operaciones aéreas.
...
def forced_operation_reason(block, opname):
    ...
    if is_aerial_operation(opname):
        ...
        if techo is not None:
            if "SALTO BASICO" in n or n == "SBM" or "LANZAMIENTO" in n or n == "LC":
                op_label = "LC" if ("LANZAMIENTO" in n or n == "LC") else "SBM"
                if techo > 0 and techo <= 3300:
                    motivos.append(f"Techo insuficiente para {op_label} <=3300 ft ({techo:g} ft)")
                elif 3300 < techo < 3500:
                    motivos.append(f"Techo marginal para {op_label} 3301-3499 ft ({techo:g} ft)")
            else:
                if techo > 0 and techo < 500:
...
def get_op_state(block, opname):
    ...
    # Para las filas operacionales principales del Gantt/PPT, el Python recalcula
    # el estado con los datos crudos del bloque. Esto evita arrastrar estados o
    # motivos antiguos del Dart, por ejemplo P con precipitación 0.0 o N con
    # techo/base de nubes favorable.
    if is_aerial_operation(opname) or "MARCHA" in target:
        return forced_operation_state(block, opname, "FAVORABLE")
...
def motivo_operacion(block, opname):
    ...
    # Para operaciones principales, usar solo motivos recalculados por Python.
    # Así la PPT no muestra limitantes antiguas si los datos del heatmap no las respaldan
    # (ej.: P con 0.0 mm o N con techo >=3500 ft en SBM/LC).
    if is_aerial_operation(opname) or "MARCHA" in target:
        return forced or ""
...
def limiting_codes(block, opname=None):
    ...
    # P solo se muestra cuando el dato real de precipitación respalda la limitante.
    # Si el heatmap dice 0.0 mm, no debe aparecer P aunque venga un motivo viejo desde el Dart.
    if precip >= 0.1:
        codes.append("P")

    # V solo cuando la visibilidad realmente cae bajo 5 km.
    # No marcar V con 9-10 km, porque operativamente no corresponde como limitante en esta PPT.
    if "VIS" in motivo or "NIEBLA" in motivo or (vis is not None and vis < 5):
        codes.append("V")

    # N = nubosidad/techo bajo solo cuando el techo/base de nubes afecta operacionalmente.
    # No se restringe por nubosidad alta si el techo operativo está favorable.
    op_norm = normalize(opname) if opname else ""
    if techo is not None and techo > 0:
        if ("SALTO BASICO" in op_norm or op_norm == "SBM" or "LANZAMIENTO" in op_norm or op_norm == "LC"):
            # Criterio usuario SBM/LC: <=3300 REST, 3301-3499 MARG, >=3500 FAV.
            if techo < 3500:
                codes.append("N")
        elif is_aerial_operation(opname):
            # Criterio aéreo general / EVACAM: techo bajo o marginal.
            if techo < 1000:
                codes.append("N")

    # Temperatura alta/fría: en Gantt solo debe marcar MARCHA/PERNOCTA.
    # No marcar T/FR en SBM, LC, SLM o EVACAM; esas filas se evalúan por viento, techo,
    # precipitación y visibilidad real.
    if temp is not None and temp >= 25 and (not opname or "MARCHA" in op_norm):
        codes.append("T")
    if temp is not None and temp <= 5 and (not opname or "MARCHA" in op_norm):
        codes.append("FR")
    if nieve > 0 and (viento >= 15 or rachas >= 20):
        codes.append("VB")
    # Oleaje: no usar "MAR" genérico porque coincide con "MARGINAL".
    if oleaje >= 1.5 or "OLEAJE" in motivo or "MAREJ" in motivo or "MAR DE FONDO" in motivo:
        codes.append("O")
...
def plain_reason_from_codes(codes):
    ...
    mapping = {
        ...
        "FR": "frío/exposición del personal",
...
def slide_heatmap(prs, data):
    full_forecast = (data.get("forecast72h", []) or [])
    forecast_chunks = chunk_forecast_by_days(full_forecast, 4)
    ...
    for r, (name, keys, unit) in enumerate(metrics, start=2):
        ...
        for c in range(1, cols):
            ...
            if n is None:
                ...
                # Criterio definido para SBM/LC: <=3300 ft restringe; 3301-3499 ft marginal; >=3500 ft favorable.
                fill = RED if n <= 3300 else ORANGE if n < 3500 else GREEN
...
def chunk_forecast_by_days(forecast, days_per_slide=4):
    if not forecast:
        return []
    ...
    for block in forecast:
        day = format_dt(block.get("hora"), "daylabel")
...
def slide_gantt(prs, data):
    ...
    for forecast in forecast_chunks:
        ...
        add_title(
            slide,
            data,
            "Gantt táctico por operación",
            "Distribución horaria de estados operacionales y limitantes principales"
        )
        set_bg(slide)
        ...
        add_text(slide, "RV = rachas/viento   P = precipitación   V = visibilidad\nN = nubosidad/techo bajo   T = temperatura   FR = frío/exposición   VB = viento blanco/nieve   O = oleaje", 3.85, 6.48, 5.1, 0.35, 7.2, False, WHITE)
...
def _reason_short(blocks, opname=None, max_items=2):
    ...
    for r in reasons[:max_items]:
        ...
        r = r.replace("Techo insuficiente para LC", "techo LC insuficiente")
        r = r.replace("Techo marginal para LC", "techo LC marginal")
...
def _daily_operational_conclusion(fecha_txt, bloques):
    ...
    if visibs and min(visibs) <= 1:
        impactos.append("visibilidad crítica: degrada observación, adquisición de blancos y control de zona")
    elif visibs and min(visibs) < 5:
        impactos.append("visibilidad reducida: limita observación y seguridad de aproximación")
    if techos and min(techos) <= 3300:
        impactos.append("techo bajo para SBM/LC: 3300 ft o menos restringe salto básico militar y lanzamiento de carga")
    elif techos and min(techos) < 3500:
        impactos.append("techo marginal para SBM/LC: 3301 a 3499 ft exige control de techo antes de ejecutar lanzamiento")
...
def _period_operation_summary_text(dias, opname, abbr, max_chars=520):
    ...
    for fecha, bloques in dias:
        ...
        if rest:
            ...
            if vmin is not None and vmin < 5:
...
def slide_resumen_operacional_periodo(prs, data):
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg(slide)
    add_title(slide, data, "Resumen operacional del período", "Síntesis final enfocada en SBM, LC, SLM, EVACAM y marcha pedestre")
    ...
    for opname, abbr, x, y, w, h in ops:
        txt = _period_operation_summary_text(dias, opname, abbr, max_chars=9999)
        worst = 0
        for _, bloques in dias:
            for b in bloques:
