...
RED = RGBColor(235, 45, 62)
RED2 = RGBColor(190, 28, 42)
...
def status_color(estado):
    """Color único y seguro para estados operacionales en PPT.
    IMPORTANTE: se evalúa RESTRINGIDO antes que MARGINAL para evitar que
    textos compuestos o variantes se pinten con un color incorrecto.
    """
    e = normalize(estado)
    if (
        "RESTRINGIDO" in e
        ...
        return RED
    if "MARGINAL" in e or e == "MARG" or "GO CON RESTRICCIONES" in e:
        return ORANGE
    if "OPTIMO" in e or "ÓPTIMO" in e:
...
def _op_thresholds(opname):
    ...
    if "SALTO LIBRE" in n or n == "SLM":
        return {"restrict": 18, "marginal": None}
    if "EVACAM" in n:
...
def _techo_ft_bloque(block):
    """Devuelve techo/base de nubes en ft desde el bloque.
    Prioriza campos ya expresados en ft; si encuentra campos en metros, convierte a ft.
    """
    if not isinstance(block, dict):
        return None
    ft_keys = [
        "techo", "techo_ft", "cloud_base", "cloud_base_ft",
        "base_nubes", "base_nubes_ft", "baseNubes", "baseNubesFt", "baseNubes_ft",
        "cloudBaseFt", "cloud_base_height_ft", "base_nubes_height_ft",
    ]
    for k in ft_keys:
        if k in block and block.get(k) not in (None, ""):
            v = valnum(block.get(k), None)
            if v is not None:
                return v
    m_keys = [
        "techo_m", "cloud_base_m", "base_nubes_m", "cloudBaseM", "baseNubesM",
        "cloud_base_height", "cloud_base_height_m", "base_nubes_height", "base_nubes_height_m",
    ]
    for k in m_keys:
        if k in block and block.get(k) not in (None, ""):
            v = valnum(block.get(k), None)
...
def _ceiling_rank_for_operation(block, opname):
    """Restricciones por techo/base de nubes en ft.
    SBM: <2500 REST; 2500-3199 MARG; >=3200 FAV por techo.
    LC: <=3400 REST; 3401-3499 MARG; >=3500 FAV por techo.
    """
    techo = _techo_ft_bloque(block)
    if techo is None or techo <= 0:
        return 0
    op_norm = normalize(opname)
    if "SALTO BASICO" in op_norm or op_norm == "SBM":
        if techo < 2500:
            return 2
        if 2500 <= techo < 3200:
            return 1
        return 0
    if "LANZAMIENTO" in op_norm or op_norm == "LC":
        if techo <= 3400:
            return 2
        if 3400 < techo < 3500:
            return 1
...
def _apply_ceiling_override(block, opname, estado):
    """Aplica override duro por techo para que el Gantt/PPT nunca ignore SBM/LC."""
    return _rank_state(max(_state_rank(estado), _ceiling_rank_for_operation(block, opname)))
...
def _aerial_forced_rank(block, opname):
    ...
    # Techo/base de nubes. Unidad: ft.
    # Criterio especial SBM: por seguridad, la aeronave debe poder subir hasta 3200 ft.
    # - <2500 ft: RESTRINGIDO
    # - 2500 a 3199 ft: MARGINAL
    # - >=3200 ft: FAVORABLE respecto a techo
    #
    # Criterio especial LC / Lanzamiento de Carga:
    # - <=3400 ft: RESTRINGIDO
    # - 3401 a 3499 ft: MARGINAL
    # - >=3500 ft: FAVORABLE respecto a techo
    if techo is not None:
        op_norm = normalize(opname)
        if "SALTO BASICO" in op_norm or op_norm == "SBM":
            if techo > 0 and techo < 2500:
                rank = max(rank, 2)
            elif 2500 <= techo < 3200:
                rank = max(rank, 1)
        elif "LANZAMIENTO" in op_norm or op_norm == "LC":
            if techo > 0 and techo <= 3400:
                rank = max(rank, 2)
            elif 3400 < techo < 3500:
                rank = max(rank, 1)
        else:
...
def _non_aerial_forced_rank(block, opname):
    ...
    Actualización PPT: temperatura <=5°C pasa a condición MARGINAL para marcha,
    permanencia/pernocta y exposición prolongada del personal.
    """
...
def forced_operation_reason(block, opname):
    ...
    if is_aerial_operation(opname):
        ...
        if techo is not None:
            if "SALTO BASICO" in n or n == "SBM":
                if techo > 0 and techo < 2500:
                    motivos.append(f"Techo insuficiente para SBM <2500 ft ({techo:g} ft)")
                elif 2500 <= techo < 3200:
                    motivos.append(f"Techo marginal para SBM 2500-3199 ft ({techo:g} ft)")
            elif "LANZAMIENTO" in n or n == "LC":
                if techo > 0 and techo <= 3400:
                    motivos.append(f"Techo insuficiente para LC <=3400 ft ({techo:g} ft)")
                elif 3400 < techo < 3500:
                    motivos.append(f"Techo marginal para LC 3401-3499 ft ({techo:g} ft)")
            else:
                if techo > 0 and techo < 500:
        ...
        if temp is not None and temp <= 5:
            motivos.append(f"Temperatura <=5°C ({temp:g}°C): exposición del personal/pernocta con limitaciones")
...
def get_op_state(block, opname):
    ...
    estado = forced_operation_state(block, opname, original)
    return _apply_ceiling_override(block, opname, estado)
...
def _safe_filename_part(value, fallback="SECTOR"):
    """Convierte texto libre del sector a un nombre legible y seguro.
    Para evitar nombres de archivo excesivos, usa solo el primer nombre del sector.
    Ej.: "Peldehue / BOE / Escuela..." -> "PELDEHUE".
    """
    raw = str(value or fallback).strip()
    if not raw:
        raw = fallback

    # Usar solo el nombre principal antes de alias extensos.
    for sep in ("/", "|", ",", ";"):
        if sep in raw:
    ...
    out = "".join(safe).strip()
    return out[:35] if out else fallback
...
def _period_for_filename(data):
    """Devuelve rango legible: 14 AL 16JUN2026 o 30JUN2026 AL 02JUL2026."""
    periodo = data.get("periodo_briefing") or {}
    forecast = data.get("forecast72h", []) or []

    ini_raw = periodo.get("inicio") or periodo.get("fecha_inicio") or periodo.get("desde")
    fin_raw = periodo.get("termino") or periodo.get("fecha_termino") or periodo.get("hasta")

    if not ini_raw and forecast:
        ini_raw = forecast[0].get("hora")
    if not fin_raw and forecast:
        fin_raw = forecast[-1].get("hora")

    ini = _parse_dt_safe(ini_raw) or _parse_period_date(ini_raw, False)
    fin = _parse_dt_safe(fin_raw) or _parse_period_date(fin_raw, False)

    if ini and fin:
        if ini.month == fin.month and ini.year == fin.year:
            return f"{ini.day:02d} AL {fin.day:02d}{_month3(fin)}{fin.year}"
        return f"{ini.day:02d}{_month3(ini)}{ini.year} AL {fin.day:02d}{_month3(fin)}{fin.year}"
    if ini:
        return f"{ini.day:02d}{_month3(ini)}{ini.year}"
    return datetime.now().strftime("%d%b%Y_%H%M").upper()


def output_file_for_data(data):
    """Nombre automático: METEO <SECTOR> <RANGO>.pptx"""
    sector = (
        data.get("sector")
    ...
    return BASE_DIR / f"METEO {sector_safe} {periodo_safe}.pptx"
...
def set_cell(cell, text, size=7, bold=False, fill=None, color=WHITE, align=PP_ALIGN.CENTER):
    ...
    if fill is not None:
        ...
        # Borde claro para evitar que algunos visores rendericen celdas rojas como negras.
        try:
...
def critical_events(data):
    ...
    if max_gust is not None and max_gust >= 13:
        estado = "RESTRINGIDO" if max_gust >= 18 else "MARGINAL"
...
def limiting_codes(block, opname=None):
    ...
    if "NUB" in motivo or nub >= 90 or (techo is not None and techo > 0 and techo < 1000):
        codes.append("N")
    if opname and _ceiling_rank_for_operation(block, opname) > 0:
        codes.append("N")
    if temp is not None and temp >= 25:
        codes.append("T")
    if temp is not None and temp <= 5:
    ...
    if nieve > 0 and (viento >= 15 or rachas >= 20):
        codes.append("VB")
    # Oleaje solo aplica a operaciones costeras/embarcadas, nunca a SBM/LC/SLM/EVACAM.
    op_norm = normalize(opname) if opname else ""
    es_costera = ("EMBARC" in op_norm or "COST" in op_norm or "PLAYA" in op_norm or "MAR" in op_norm)
    if es_costera and (oleaje >= 1.5 or "OLEAJE" in motivo or "MAR" in motivo):
        codes.append("O")
...
def plain_reason_from_codes(codes):
    ...
    mapping = {
        ...
        "TF": "frío/exposición del personal",
    ...
    for r, (name, keys, unit) in enumerate(metrics, start=2):
        ...
        for c in range(1, cols):
            ...
            if n is None:
                ...
                fill = RED if n >= 18 else ORANGE if n >= 13 else GREEN
            elif name == "RACHAS":
                fill = RED if n >= 18 else ORANGE if n >= 13 else GREEN
            elif name == "PRECIP":
                ...
                # Criterio de seguridad para SBM: <2500 ft restringe; 2500-3100 ft marginal; >=3200 ft favorable.
                fill = RED if n < 2500 else ORANGE if n < 3200 else GREEN
            cell_font_size = 9 if name == "TECHO" else 10.5
            set_cell(table.cell(r, c), label, cell_font_size, True, fill)
...
def _clock_status_color(est):
    """Color para imágenes PIL del reloj táctico.
    Se usa rojo claro y no negro para restricciones.
    """
    s = normalize(est)
    if "RESTRING" in s or "REST" == s or "NO GO" in s or "NOGO" in s:
        return (235, 45, 62)
    if "MARG" in s or "GO CON RESTRICCIONES" in s:
        return (239, 145, 38)
    if "OPT" in s:
        return (40, 205, 126)
    if "FAV" in s or s == "GO":
...
def _make_tactical_clock_image(data, out_path):
    ...
    # Leyenda
    legend = [("ÓPTIMO/FAV", (38,180,100)), ("MARGINAL", (239,145,38)), ("RESTRINGIDO", (235,45,62)), ("SIN EVAL.", (100,112,128))]
    y = 470
    for label, col in legend:
...
def slide_gantt(prs, data):
    forecast = (data.get("forecast72h", []) or [])
    incluir_aerea = bool(data.get("incluir_aerea", True))
    incluir_costera = bool(data.get("incluir_costera", data.get("costera") is not None)) and bool(data.get("costera"))
    incluir_montana = bool(data.get("incluir_montana", True)) and bool(data.get("montana"))

    ops = []
    labels = []
    if incluir_aerea:
        ops.extend(["SALTO BASICO", "LANZAMIENTO", "SALTO LIBRE", "EVACAM"])
        labels.extend(["SBM", "LC", "SLM", "EVACAM"])

    # Marcha/pernocta queda como evaluación terrestre general cuando hay modo aéreo o montaña.
    if incluir_aerea or incluir_montana:
        ops.append("MARCHA")
        labels.append("MARCHA")
    ...
    # Cada fila ya identifica la operación (SBM, LC, SLM, EVACAM, etc.).
    # Por eso las celdas NO repiten la operación: muestran solo la limitante
    # meteorológica que provoca condición marginal/restringida.
    # Ej.: RV = rachas/viento; RV/P = rachas/viento + precipitación.
    for r, (op, lab) in enumerate(zip(ops, labels), start=2):
        set_cell(table.cell(r, 0), lab, 10, True, PANEL3, align=PP_ALIGN.LEFT)
        for c in range(1, cols):
            b = forecast[c-1] if c-1 < len(forecast) else {}
            est = _apply_ceiling_override(b, op, get_op_state(b, op))
            codes = limiting_codes(b, op) if estado_score(est) > 0 else ""
            set_cell(table.cell(r, c), codes, 7.6, True, status_color(est))
    ...
    add_text(slide, "RV = rachas/viento   P = precipitación   V = visibilidad\nN = nubosidad/techo   T = temperatura   VB = viento blanco/nieve   O = oleaje (costero)", 3.85, 6.48, 5.1, 0.35, 7.2, False, WHITE)
...
def _reason_short(blocks, opname=None, max_items=2):
    ...
    for r in reasons[:max_items]:
        ...
        r = r.replace("Techo insuficiente para SBM", "techo SBM insuficiente")
        r = r.replace("Techo marginal para SBM", "techo SBM marginal")
        ...
        r = r.replace("Temperatura <=5°C", "T° <=5°C")
        r = r.replace("Temperatura >=25°C", "T° >=25°C")
        cleaned.append(r)
...
def _daily_operational_conclusion(fecha_txt, bloques):
    ...
    if all_aerial_rest:
        lines.append(
            f"Aéreas/evacuación: RESTRINGIDO en {_compress_windows(all_aerial_rest)} por "
        ...
        for op, abbr in aerial_ops:
            ...
            if rest:
                restricciones.append(
                    f"{abbr} restringido {_compress_windows(rest)} ({_reason_short(rest, op)})"
                )
        if restricciones:
    ...
    if all_aerial_fav:
        ...
        if fav_parts:
            ...
            lines.append("No se aprecia ventana aérea favorable continua; planificar alternativa terrestre o reprogramar.")

    # 2) Impacto operacional y exposición del personal.
    ...
    if rachas and max(rachas) >= 13:
        impactos.append("rachas relevantes: incrementan riesgo para paracaidistas y obligan a corrección/reevaluación de lanzamiento o maniobra")
    ...
    if techos and min(techos) < 2500:
        impactos.append("techo bajo para SBM: no permite margen de seguridad para ascenso a 3200 ft y afecta zona de caída")
    elif techos and min(techos) < 3200:
        impactos.append("techo marginal para SBM: exige control de techo antes de ejecutar salto básico")
    if techos and min(techos) <= 3400:
        impactos.append("techo restringido para LC: techo <=3400 ft no cumple criterio para lanzamiento de carga")
    elif techos and min(techos) < 3500:
        impactos.append("techo marginal para LC: rango 3401-3499 ft exige reevaluación antes del lanzamiento de carga")
    ...
    if temps:
        if min(temps) <= 5:
            impactos.append(f"frío operacional: T° mínima {min(temps):.1f}°C; compromete marcha, espera prolongada, pernocta, abrigo, baterías y manipulación de equipo")
        elif max(temps) >= 25:
    ...
    if estado == "RESTRINGIDO":
        lines.append("Decisión sugerida: suspender bloques restringidos, emplear ventanas favorables puntuales y exigir actualización previa a la orden de ejecución.")
    elif estado == "MARGINAL":
...
def _min_value(blocks, keys):
    vals = []
    for b in blocks or []:
        v = valnum(get_metric(b, keys, None), None)
        if v is not None:
            vals.append(v)
    return min(vals) if vals else None


def _max_value(blocks, keys):
    vals = []
    for b in blocks or []:
        v = valnum(get_metric(b, keys, None), None)
        if v is not None:
...
def _fmt_metric(value, dec=1, suffix=""):
    if value is None:
        return "N/D"
    try:
        return f"{float(value):.{dec}f}{suffix}"
    except Exception:
        return str(value)


def _period_operation_summary_text(dias, opname, abbr, max_chars=520):
    restricted_entries = []
    marginal_entries = []
    ...
    for fecha, bloques in dias:
        ...
        rest = _op_restricted_window(bloques_validos, opname)
        marg = _op_marginal_window(bloques_validos, opname)

        if rest:
            rmax = _max_value(rest, ["rachas"])
            vmax = _max_value(rest, ["viento"])
            vmin = _min_value(rest, ["visibilidad"])
            tmin = _min_value(rest, ["temperatura"])
            reason = _reason_short(rest, opname, max_items=2)
            datos = []
            if rmax is not None:
                datos.append(f"racha máx {_fmt_metric(rmax, 1, ' kt')}")
            elif vmax is not None:
                datos.append(f"viento máx {_fmt_metric(vmax, 1, ' kt')}")
            if vmin is not None and vmin <= 10:
                datos.append(f"vis mín {_fmt_metric(vmin, 1, ' km')}")
            if "MARCHA" in normalize(opname) and tmin is not None and tmin <= 5:
                datos.append(f"T° mín {_fmt_metric(tmin, 1, '°C')}")
            detalle = "; ".join(datos)
            restricted_entries.append(
                f"{fecha} ({_compress_windows(rest)}; {reason}{'; ' + detalle if detalle else ''})"
            )
        elif marg:
            rmax = _max_value(marg, ["rachas"])
            tmin = _min_value(marg, ["temperatura"])
            reason = _reason_short(marg, opname, max_items=2)
            datos = []
            ...
            marginal_entries.append(
                f"{fecha} ({_compress_windows(marg)}; {reason}{'; ' + detalle if detalle else ''})"
            )

    if restricted_entries:
        txt = f"No recomendable/restringido: " + "; ".join(restricted_entries[:5]) + "."
        if len(restricted_entries) > 5:
            txt += f" Se agregan {len(restricted_entries)-5} día(s) con restricción similar."
    elif marginal_entries:
        txt = f"Con limitaciones: " + "; ".join(marginal_entries[:5]) + "."
        if len(marginal_entries) > 5:
            txt += f" Se agregan {len(marginal_entries)-5} día(s) marginal(es)."
    else:
        txt = "Sin restricción operacional relevante detectada para el período; mantener monitoreo de rachas, visibilidad, techo y temperatura antes de ejecutar."

    return text_wrap(txt, max_chars)


def _overall_period_recommendation(dias):
    total_days = len(dias)
    if total_days == 0:
        return "Sin datos suficientes para generar resumen del período."
    ...
    for fecha, bloques in dias:
        ...
        if not bloques_validos:
            continue
        states.append(_day_state_from_blocks(bloques_validos))
        tmin = _min_value(bloques_validos, ["temperatura"])
        if tmin is not None and tmin <= 5:
            cold_days.append(f"{fecha} ({tmin:.1f}°C)")

    rest_count = sum(1 for st in states if st == "RESTRINGIDO")
    marg_count = sum(1 for st in states if st == "MARGINAL")

    if rest_count:
        base = f"Período con predominio de restricciones operacionales ({rest_count} día(s) restringido(s) y {marg_count} marginal(es)). No planificar operaciones aéreas críticas como esfuerzo principal sin actualización meteorológica previa."
    elif marg_count:
        base = f"Período con condiciones marginales en {marg_count} día(s). Ejecutar solo con control horario, mitigaciones y plan alterno."
    else:
        base = "Período mayoritariamente favorable; mantener control de cambios locales y actualización antes de la ejecución."

    if cold_days:
        base += " Temperatura: considerar limitaciones por frío/exposición del personal en " + ", ".join(cold_days[:5]) + "."

    base += " Priorizar ventanas favorables puntuales, mantener alternativa terrestre y verificar rachas/visibilidad/techo antes de la orden de ejecución."
    return text_wrap(base, 520)


def slide_resumen_operacional_periodo(prs, data):
    forecast = data.get("forecast72h", []) or []
    dias = list(grouped_by_date(forecast).items())
    ...
    enfoque = []
    if incluir_aerea:
        enfoque.append("SBM, LC, SLM y EVACAM")
    if incluir_montana:
        enfoque.append("montaña")
    if incluir_costera:
        enfoque.append("modo costero")
    enfoque.append("marcha/pernocta")
    add_title(slide, data, "Resumen operacional del período", "Síntesis final enfocada en " + ", ".join(enfoque))

    if not dias:
        add_text(slide, "Sin datos de pronóstico disponibles para resumir el período.", 0.55, 1.8, 12, 0.5, 14, True, RED)
        return

    add_rect(slide, 0.35, 1.48, 12.65, 0.78, PANEL2, BLUE)
    add_text(slide, "CONCLUSIÓN GENERAL", 0.62, 1.62, 2.7, 0.22, 10.5, True, CYAN)
    add_text(slide, _overall_period_recommendation(dias), 0.62, 1.88, 12.0, 0.28, 8.3, False, WHITE)

    if incluir_aerea:
        ops = [
            ("Salto basico militar", "SBM", 0.35, 2.48, 4.05, 1.36),
            ("Lanzamiento de carga", "LC", 4.65, 2.48, 4.05, 1.36),
            ("Salto libre militar", "SLM", 8.95, 2.48, 4.05, 1.36),
            ("EVACAM", "EVACAM", 0.35, 4.10, 6.10, 1.38),
            ("Marcha pedestre", "MARCHA / PERNOCTA", 6.90, 4.10, 6.10, 1.38),
        ]
    else:
        ops = [
            ("Marcha pedestre", "MARCHA / PERNOCTA", 0.65, 2.65, 12.0, 1.55),
        ]

    for opname, abbr, x, y, w, h in ops:
        txt = _period_operation_summary_text(dias, opname, abbr, max_chars=360 if w < 5 else 500)
        worst = 0
        for _, bloques in dias:
            for b in bloques:
                worst = max(worst, _state_rank(get_op_state(b, opname)))
        color = RED if worst >= 2 else ORANGE if worst == 1 else GREEN
        add_rect(slide, x, y, w, h, PANEL2, color)
        add_badge(slide, abbr, x + 0.14, y + 0.14, 1.38 if len(abbr) > 4 else 0.82, 0.28, color)
        title_x = x + (1.62 if len(abbr) > 4 else 1.08)
        add_text(slide, "Resumen por operación", title_x, y + 0.16, w - (title_x - x) - 0.2, 0.20, 8.6, True, MUTED)
        add_text(slide, txt, x + 0.16, y + 0.52, w - 0.32, h - 0.62, 7.3 if w < 5 else 7.6, False, WHITE)

    add_rect(slide, 0.35, 5.78, 12.65, 0.82, PANEL, RGBColor(70, 90, 115))
    add_text(slide, "CRITERIO TEMPERATURA PERSONAL", 0.62, 5.92, 3.2, 0.22, 9.5, True, YELLOW)
    add_text(slide, "Temperatura ≤5°C: condición MARGINAL para marcha pedestre, espera prolongada y pernocta fuera de unidad. Requiere abrigo, control de exposición, verificación de baterías/equipo, hidratación y plan de evacuación. Temperaturas bajo 0°C aumentan exigencia fisiológica y riesgo de congelamiento localizado.", 0.62, 6.18, 12.0, 0.25, 7.8, False, WHITE)

    add_text(slide, "Resumen automático generado desde los bloques meteorológicos del período seleccionado.", 0.50, 6.88, 12.0, 0.18, 8.0, False, MUTED)



def _sol_luna_items(data):
    items = data.get("sol_luna") or data.get("astronomia") or []
    if isinstance(items, dict):
        items = items.get("periodo") or items.get("dias") or []
    if not isinstance(items, list):
        return []

    clean = [x for x in items if isinstance(x, dict)]

    # Respeta las fechas seleccionadas para el briefing.
    # Evita que la slide Sol/Luna muestre los 10/16 días completos del modelo
    # cuando el operador seleccionó un rango menor.
    inicio, termino = _period_bounds(data)
    if not inicio and not termino:
        return clean
    ...
    out = []
    for item in clean:
        raw_fecha = item.get("fecha") or item.get("date") or item.get("dia")
        dt = _parse_period_date(raw_fecha, False)
        if not dt:
            # Si el ítem no trae fecha, se conserva para no ocultar información por error de formato.
            out.append(item)
            continue
        if inicio and dt < inicio:
            continue
...
def _sl_float(item, *keys, default=None):
    for k in keys:
        if k in item:
            v = valnum(item.get(k), None)
            if v is not None:
                return v
    return default


def _sol_luna_fecha_label(item):
    fecha = item.get("fecha") or item.get("date") or item.get("dia") or ""
    if fecha:
        return format_dt(fecha, "date")
    return _sl_get(item, "fecha_label", "label", default="DÍA")


def slide_sol_luna_crepusculos(prs, data):
    items = _sol_luna_items(data)
    if not items:
        return

    grupos = [items[i:i+4] for i in range(0, len(items), 4)]
    for idx, grupo in enumerate(grupos):
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        set_bg(slide)
        subt = "Sol, luna y crepúsculos por día"
        if len(grupos) > 1:
            subt += f" · Continuación {idx+1}/{len(grupos)}"
        add_title(slide, data, "Sol / Luna y crepúsculos", subt)
        ...
        for i, item in enumerate(grupo):
            x = xs[i]
            fecha = _sol_luna_fecha_label(item)
            add_rect(slide, x, 1.55, w, 5.35, PANEL2, RGBColor(70, 90, 115))
            add_badge(slide, fecha, x + 0.12, 1.72, min(1.42, w-0.25), 0.30, BLUE)

            salida = _sl_get(item, "salida_sol", "sunrise")
            puesta = _sl_get(item, "puesta_sol", "sunset")
            civil_i = _sl_get(item, "crepusculo_civil_inicio", "civil_twilight_begin")
            civil_f = _sl_get(item, "crepusculo_civil_fin", "civil_twilight_end")
            naut_i = _sl_get(item, "crepusculo_nautico_inicio", "nautical_twilight_begin")
            naut_f = _sl_get(item, "crepusculo_nautico_fin", "nautical_twilight_end")
            astro_i = _sl_get(item, "crepusculo_astronomico_inicio", "astronomical_twilight_begin")
            astro_f = _sl_get(item, "crepusculo_astronomico_fin", "astronomical_twilight_end")
            fase = _sl_get(item, "fase_lunar", "fase", "moon_phase")
            ilum = _sl_float(item, "luna_iluminacion", "iluminacion", "moon_illumination")
            edad = _sl_float(item, "edad_lunar_dias", "edad_dias", "moon_age")
            horas_luz = _sl_float(item, "horas_luz", "daylight_hours")
            horas_sol = _sl_float(item, "horas_sol", "sunshine_hours")

            y = 2.18
            add_text(slide, "SOL", x+0.16, y, w-0.32, 0.20, 9.2, True, CYAN)
            y += 0.26
            add_text(slide, f"Salida: {salida}\nPuesta: {puesta}", x+0.16, y, w-0.32, 0.48, 8.2, False, WHITE)
            y += 0.58
            if horas_luz is not None or horas_sol is not None:
                luz_txt = f"Luz: {horas_luz:.1f} h" if horas_luz is not None else "Luz: -"
                sol_txt = f"Sol: {horas_sol:.1f} h" if horas_sol is not None else "Sol: -"
                add_text(slide, f"{luz_txt} · {sol_txt}", x+0.16, y, w-0.32, 0.20, 7.5, False, MUTED)
                y += 0.28

            add_text(slide, "CREPÚSCULOS", x+0.16, y, w-0.32, 0.20, 9.0, True, YELLOW)
            y += 0.26
            add_text(slide,
                     f"Astronómico: {astro_i} / {astro_f}\nNáutico: {naut_i} / {naut_f}\nCivil: {civil_i} / {civil_f}",
                     x+0.16, y, w-0.32, 0.76, 7.4, False, WHITE)
            y += 0.92

            add_text(slide, "LUNA", x+0.16, y, w-0.32, 0.20, 9.0, True, CYAN)
            y += 0.26
            ilum_txt = f"{ilum:.1f}%" if ilum is not None else "N/D"
            edad_txt = f"{edad:.1f} días" if edad is not None else "N/D"
            add_text(slide, f"Fase: {text_wrap(fase, 75)}\nLuminosidad: {ilum_txt}\nEdad: {edad_txt}", x+0.16, y, w-0.32, 0.72, 7.5, False, WHITE)
            y += 0.86

            estado_luna, eval_luna = lunar_eval_from_illum(ilum)
            eval_txt = text_wrap(eval_luna, 150 if w > 5 else 95)
            eval_color = GREEN if estado_luna in ("ÓPTIMA PARA INFIL", "FAVORABLE") else ORANGE if estado_luna == "MARGINAL" else RED if estado_luna == "DESFAVORABLE" else MUTED
            add_text(slide, f"Condición nocturna: {estado_luna}", x+0.16, y, w-0.32, 0.18, 7.2, True, eval_color)
            y += 0.24
            add_text(slide, eval_txt, x+0.16, y, w-0.32, 0.45, 6.8, False, MUTED)

        add_text(slide,
                 "Los horarios de crepúsculo son aproximados y se calculan para la coordenada consultada. Usar como apoyo de planificación nocturna, observación y movimiento.",
                 0.45, 7.02, 12.4, 0.20, 7.4, False, MUTED)
...
def main():
    ...
    slide_sol_luna_crepusculos(prs, data)
    slide_gantt(prs, data)
    # Se elimina la diapositiva de tarjetas críticas para evitar duplicidad.
    slide_costera(prs, data)
    slide_montana(prs, data)
    slide_municion(prs, data)
    slide_conclusiones(prs, data)
    slide_resumen_operacional_periodo(prs, data)
