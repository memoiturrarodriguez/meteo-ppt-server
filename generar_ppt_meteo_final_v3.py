...
def is_airdrop_operation(opname):
    n = normalize(opname)
    return (
        "SALTO BASICO" in n or
        "LANZAMIENTO" in n or
...
def is_freefall_operation(opname):
    n = normalize(opname)
    return "SALTO LIBRE" in n or n == "SLM"


def is_static_line_or_cargo_operation(opname):
    n = normalize(opname)
    return "SALTO BASICO" in n or "LANZAMIENTO" in n or n in ("SBM", "LC")
...
def _aerial_forced_rank(block, opname):
    ...
    # Techo/base de nubes.
    # SBM/LC mantienen criterio de seguridad por línea estática/carga:
    # - <=3300 ft: RESTRINGIDO
    # - 3301 a 3499 ft: MARGINAL
    # - >=3500 ft: FAVORABLE respecto a techo
    # Salto libre militar se evalúa distinto: restringe solo con techo <=1000 ft.
    if techo is not None:
        if is_freefall_operation(opname):
            if techo > 0 and techo <= 1000:
                rank = max(rank, 2)
        elif is_static_line_or_cargo_operation(opname):
...
def forced_operation_reason(block, opname):
    ...
    if is_aerial_operation(opname):
        ...
        if techo is not None:
            if is_freefall_operation(opname):
                if techo > 0 and techo <= 1000:
                    motivos.append(f"Techo restringido <=1000 ft ({techo:g} ft)")
            elif is_static_line_or_cargo_operation(opname):
                if techo > 0 and techo <= 3300:
                    motivos.append(f"Techo restringido <=3300 ft ({techo:g} ft)")
                elif 3300 < techo < 3500:
                    motivos.append(f"Techo marginal 3301-3499 ft ({techo:g} ft)")
            else:
                if techo > 0 and techo < 500:
...
def limiting_codes(block, opname=None):
    ...
    if "NUB" in motivo or nub >= 90 or (techo is not None and techo > 0 and techo < 1000):
        codes.append("N")
    if opname and techo is not None:
        if is_freefall_operation(opname) and 0 < techo <= 1000:
            codes.append("N")
        elif is_static_line_or_cargo_operation(opname) and 0 < techo < 3500:
            codes.append("N")
    if temp is not None and temp >= 25:
        codes.append("T")
    if temp is not None and temp <= 5:
    ...
    for r, (name, keys, unit) in enumerate(metrics, start=2):
        ...
        for c in range(1, cols):
            ...
            if n is None:
                ...
                # Criterio de seguridad para SBM/LC: <=3300 ft restringe; 3301-3499 ft marginal; >=3500 ft favorable. SLM se evalúa en su tabla operacional con <=1000 ft.
...
def _modulos_briefing(data):
    mods = data.get("modulos_briefing") or {}
    return {
        "aereas": bool(mods.get("operaciones_aereas", data.get("incluir_operaciones_aereas", True))),
        "costera": bool(mods.get("costera", data.get("incluir_costera", bool(data.get("costera"))))),
        "montana": bool(mods.get("montana", data.get("incluir_montana", True))),
    }


def slide_gantt(prs, data):
    forecast = (data.get("forecast72h", []) or [])
    mods = _modulos_briefing(data)
    incluir_aereas = mods["aereas"]
    incluir_costera = mods["costera"] and bool(data.get("costera"))
    incluir_montana = mods["montana"] and bool(data.get("montana"))

    ops = []
    labels = []
    if incluir_aereas:
        ops.extend(["SALTO BASICO", "LANZAMIENTO", "SALTO LIBRE", "EVACAM", "MARCHA"])
        labels.extend(["SBM", "LC", "SLM", "EVACAM", "MARCHA"])
    if incluir_costera:
        ops.append("EMBARC")
        labels.append("EMB")
    if incluir_montana:
...
def slide_costera(prs, data):
    if not _modulos_briefing(data).get("costera", False):
        return
...
def slide_montana(prs, data):
    if not _modulos_briefing(data).get("montana", True):
        return
...
def _reason_short(blocks, opname=None, max_items=2):
    ...
    for r in reasons[:max_items]:
        ...
        r = r.replace("Techo restringido", "techo restringido")
        r = r.replace("Techo marginal", "techo marginal")
...
def _daily_operational_conclusion(fecha_txt, bloques):
    ...
    if techos and min(techos) <= 3300:
        impactos.append("techo restringido para SBM/LC: no cumple criterio mínimo de 3500 ft")
    elif techos and min(techos) < 3500:
        impactos.append("techo marginal para SBM/LC: exige control de techo antes de ejecutar")
...
def _date_label_from_any(value):
    """Devuelve etiqueta 08JUN2026 desde ISO/yyyy-mm-dd/dd-mm-yyyy si es posible."""
    if not value:
        return ""
    try:
        return format_dt(value, "date")
    except Exception:
        return str(value or "")[:10]


def _selected_day_labels_for_sol_luna(data):
    """Días que deben mostrarse en Sol/Luna según el período solicitado.

    La app puede enviar astronomía para el máximo de días descargados
    (por ejemplo 16), pero la PPT debe mostrar solo los días elegidos
    por el operador en el calendario. Como `apply_period_filter()` ya deja
    `forecast72h` exacto al período solicitado, usamos primero esos días.
    """
    forecast = data.get("forecast72h", []) or []
    labels = []
    for b in forecast:
        if not isinstance(b, dict):
            continue
        label = _date_label_from_any(b.get("hora") or b.get("fecha") or b.get("date"))
    ...
    # Respaldo si no hay forecast, pero sí período briefing.
    inicio, termino = _period_bounds(data)
    if inicio and termino:
        out = set()
        d = datetime(inicio.year, inicio.month, inicio.day)
        last = datetime(termino.year, termino.month, termino.day)
        while d <= last:
            out.add(format_dt(d.isoformat(), "date"))
...
def _sol_luna_items(data):
    ...
    allowed = _selected_day_labels_for_sol_luna(data)
    if not allowed:
        return items

    filtrados = []
    for item in items:
        fecha_raw = item.get("fecha") or item.get("date") or item.get("dia") or item.get("hora")
        label = _date_label_from_any(fecha_raw)
        # Si el item no trae fecha reconocible, se descarta para no llenar la PPT con días fuera del período.
        if label in allowed:
...
def main():
    ...
    mods = _modulos_briefing(data)
    incluir_aereas = mods["aereas"]
    incluir_costera = mods["costera"]
    incluir_montana = mods["montana"]

    slide_portada(prs, data)

    # La presentación ahora respeta los módulos solicitados desde la app:
    # Aéreas, Costero y/o Montaña.
    if incluir_aereas:
        slide_heatmap(prs, data)
        slide_sol_luna_crepusculos(prs, data)

    if incluir_aereas or incluir_costera or incluir_montana:
    ...
    # Tiro/munición queda fuera del flujo estándar para privilegiar resumen operacional.
    if incluir_aereas:
