...
def _aerial_forced_rank(block, opname):
    ...
    techo = valnum(get_metric(block, ["techo", "techo_ft", "cloud_base", "cloud_base_ft", "base_nubes", "base_nubes_ft", "baseNubes", "baseNubesFt", "baseNubes_ft"], None), None)
...
def limiting_codes(block, opname=None):
    ...
    if "NUB" in motivo or nub >= 90 or (techo is not None and techo > 0 and techo < 1000):
        codes.append("N")
    if opname and ("SALTO BASICO" in normalize(opname) or normalize(opname) == "SBM") and techo is not None and 0 < techo < 3200:
        codes.append("N")
    if opname and ("LANZAMIENTO" in normalize(opname) or normalize(opname) == "LC") and techo is not None and 0 < techo < 3500:
        codes.append("N")
    if temp is not None and temp >= 25:
        codes.append("T")
    if temp is not None and temp <= 5:
