...
def _aerial_forced_rank(block, opname):
    ...
    # Techo/base de nubes.
    # Criterio para operaciones de lanzamiento/paracaidismo:
    # - <=3300 ft: RESTRINGIDO
    # - 3301 a 3499 ft: MARGINAL
    # - >=3500 ft: FAVORABLE respecto a techo
    if techo is not None:
        if is_airdrop_operation(opname):
...
def limiting_codes(block, opname=None):
    ...
    if "NUB" in motivo or nub >= 90 or (techo is not None and techo > 0 and techo < 1000):
        codes.append("N")
    if opname and is_airdrop_operation(opname) and techo is not None and 0 < techo < 3500:
        codes.append("N")
    if temp is not None and temp >= 25:
        codes.append("T")
    if temp is not None and temp <= 5:
    ...
    for r, (name, keys, unit) in enumerate(metrics, start=2):
        ...
        for c in range(1, cols):
            ...
            if n is None:
                ...
                # Criterio de seguridad para operaciones de lanzamiento/paracaidismo: <=3300 ft restringe; 3301-3499 ft marginal; >=3500 ft favorable.
...
def _daily_operational_conclusion(fecha_txt, bloques):
    ...
    if techos and min(techos) <= 3300:
        impactos.append("techo restringido para operaciones de lanzamiento/paracaidismo: no cumple criterio mínimo de 3500 ft")
    elif techos and min(techos) < 3500:
        impactos.append("techo marginal para operaciones de lanzamiento/paracaidismo: exige control de techo antes de ejecutar")
