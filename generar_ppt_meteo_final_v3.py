...
def slide_heatmap(prs, data):
    ...
    if forecast:
        ...
        while c < cols:
            ...
            set_cell(table.cell(0, start_c), day, 9, True, DARK)
    ...
    for r, (name, keys, unit) in enumerate(metrics, start=2):
        set_cell(table.cell(r, 0), name, 9, True, PANEL3, align=PP_ALIGN.LEFT)
        for c in range(1, cols):
            ...
            set_cell(table.cell(r, c), label, 9, True, fill)
...
def slide_montana(prs, data):
    if not (bool(data.get("incluir_montana", True)) and data.get("montana")):
        return
    forecast = data.get("forecast72h", []) or []
    montana = data.get("montana", {}) or {}
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg(slide)
    add_title(slide, data, "Módulo montaña operacional", "Riesgos de montaña, nieve, isoterma y viento blanco")

    estado = montana.get("estado", "SIN EVALUAR")
    add_badge(slide, estado, 10.15, 1.55, 2.5, 0.45, status_color(estado))
    add_rect(slide, 0.45, 1.55, 9.35, 1.0, PANEL2)
    add_text(slide, "CONDICIÓN GENERAL", 0.65, 1.72, 2.5, 0.2, 9, True, CYAN)
    add_text(slide, text_wrap(montana.get("motivo", "Sin datos específicos de montaña."), 180), 0.65, 1.98, 8.8, 0.35, 10, False, WHITE)

    risks = []
    for b in forecast:
        viento = valnum(b.get("viento"), 0)
        rachas = valnum(b.get("rachas"), 0)
        nieve = valnum(b.get("nevadas", b.get("snowfall")), 0)
        vis = valnum(b.get("visibilidad"), None)
        temp = valnum(b.get("temperatura"), None)
        iso = valnum(b.get("isoterma", b.get("freezing_level_height")), None)
        if (nieve and nieve > 0) and (rachas and rachas >= 20 or viento and viento >= 15):
            risks.append(("VIENTO BLANCO", b, "Nieve + viento/rachas relevantes"))
        elif temp is not None and temp <= 0:
            risks.append(("CONGELAMIENTO", b, "Temperatura igual o inferior a 0°C"))
        elif vis is not None and vis <= 5:
            risks.append(("VISIBILIDAD REDUCIDA", b, "Visibilidad baja en montaña"))
        elif iso is not None and iso < 1500:
            risks.append(("ISOTERMA BAJA", b, "Isoterma 0 baja para movilidad en altura"))
    risks = risks[:5]

    add_text(slide, "RIESGOS AUTOMÁTICOS", 0.55, 2.85, 4.0, 0.25, 12, True, CYAN)
    if not risks:
        add_rect(slide, 0.55, 3.25, 12.2, 0.7, PANEL2)
        add_text(slide, "Sin riesgos críticos automáticos detectados para montaña. Mantener actualización previa a ejecución.", 0.75, 3.47, 11.7, 0.22, 10, False, WHITE)
    else:
        for i, (name, b, detail) in enumerate(risks):
            y = 3.15 + i * 0.62
            add_rect(slide, 0.55, y, 12.2, 0.5, PANEL2)
            add_badge(slide, "RIESGO", 0.75, y+0.11, 0.8, 0.22, ORANGE)
            add_text(slide, f"{name} · {format_dt(b.get('hora'), 'daylabel')} {format_dt(b.get('hora'))}h", 1.7, y+0.09, 3.8, 0.2, 8.8, True, WHITE)
            add_text(slide, detail, 5.6, y+0.09, 6.7, 0.2, 8.5, False, MUTED)

    add_rect(slide, 0.55, 6.55, 12.2, 0.45, PANEL)
    add_text(slide, "Criterios referenciales: viento blanco = nieve + viento/rachas relevantes. Confirmar con observación local y fuentes oficiales.", 0.75, 6.68, 11.8, 0.18, 8, False, MUTED)
