...
def _safe_filename_part(value, fallback="SECTOR"):
    """Convierte texto libre del sector a un nombre corto y seguro para el archivo.

    Regla SOW: usar solo el primer nombre/tramo del sector.
    Ej.: "Peldehue / BOE Lautaro / Escuela Paracaidistas, Colina..." -> "PELDEHUE".
    """
    raw = str(value or fallback).strip()
    if not raw:
        raw = fallback

    # Usar solo el primer tramo real del sector: antes de coma, slash, guion largo o separadores.
    for sep in [",", "/", "|", " - ", " – ", " — "]:
        if sep in raw:
    ...
    out = "".join(safe).strip()
    return out[:35].strip() if out else fallback


def _parse_dt_for_filename(value):
    try:
        return datetime.fromisoformat(str(value or "").replace("Z", "")[:19])
    except Exception:
        try:
...
def _period_for_filename(data):
    """Formato SOW: 14 AL 16JUN2026 o 30MAY2026 AL 02JUN2026."""
    periodo = data.get("periodo_briefing") or {}
    ...
    dt_ini = _parse_dt_for_filename(ini)
    dt_fin = _parse_dt_for_filename(fin)
    meses = ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"]

    if dt_ini and dt_fin:
        if dt_ini.month == dt_fin.month and dt_ini.year == dt_fin.year:
            return f"{dt_ini.day:02d} AL {dt_fin.day:02d}{meses[dt_fin.month-1]}{dt_fin.year}"
        return f"{dt_ini.day:02d}{meses[dt_ini.month-1]}{dt_ini.year} AL {dt_fin.day:02d}{meses[dt_fin.month-1]}{dt_fin.year}"
    if dt_ini:
        return f"{dt_ini.day:02d}{meses[dt_ini.month-1]}{dt_ini.year}"
    return datetime.now().strftime("%d%b%Y %H%M").upper()


def output_file_for_data(data):
    """Nombre automático: METEO <SECTOR_CORTO> <PERIODO>.pptx"""
    sector = (
        data.get("sector")
...
def _op_permite_oleaje(opname):
    """El oleaje/estado de mar solo aplica a modo costero o embarcación, nunca a saltos/aéreas."""
    t = normalize(opname or "")
    return any(x in t for x in ["EMBARC", "COSTER", "MARIT", "OLEAJE", "PLAYA", "PUMAR", "BOTE"])
...
def limiting_codes(block, opname=None):
    ...
    if "NUB" in motivo or nub >= 90 or (techo is not None and techo > 0 and techo < 1000):
        codes.append("N")
    if opname and ("SALTO BASICO" in normalize(opname) or normalize(opname) == "SBM" or "LANZAMIENTO" in normalize(opname) or normalize(opname) == "LC") and techo is not None and 0 < techo < 3500:
        codes.append("N")
    if temp is not None and temp >= 25:
        codes.append("T")
    if temp is not None and temp <= 5:
    ...
    if nieve > 0 and (viento >= 15 or rachas >= 20):
        codes.append("VB")
    # Oleaje/estado de mar NO aplica a saltos ni operaciones aéreas.
    # Solo se informa en operación costera/embarcación.
    if _op_permite_oleaje(opname) and (oleaje >= 1.5 or "OLEAJE" in motivo or "MAR" in motivo):
        codes.append("O")
...
def slide_gantt(prs, data):
    ...
    add_text(slide, "RV = rachas/viento   P = precipitación   V = visibilidad\nN = nubosidad/techo   T = temperatura   VB = viento blanco/nieve   O = oleaje", 3.85, 6.48, 5.1, 0.35, 7.2, False, WHITE)
