        ...
        if techo is not None:
            if "SALTO BASICO" in n or n == "SBM":
                if techo > 0 and techo < 2500:
                    ...
                    motivos.append(f"Techo marginal para SBM 2500-3100 ft ({techo:g} ft)")
            else:
                if techo > 0 and techo < 500:
...
def limiting_codes(block, opname=None):
    """Devuelve SOLO abreviaciones de la limitante meteorológica para la celda Gantt.

    La fila ya identifica la operación (SBM, LC, SLM, EVACAM, MARCHA), por lo que
    la celda nunca debe repetir la operación. Si el bloque está marginal/restringido,
    la celda debe mostrar la causa meteorológica: RV/P/V/N/T/TF/VB/O. Si por alguna
    razón el JSON no trae motivo ni variables suficientes, usa MET como respaldo para
    evitar cuadros rojos/naranjos vacíos.
    """
    opn = normalize(opname or "")
    ...
    # 1) Leer el motivo textual de la operación, si viene desde la app o si fue forzado por PPT.
    if any(x in motivo for x in ["VIENTO", "RACHA", "RACHAS", "NUDO", "KT"]):
        add("RV")
    if any(x in motivo for x in ["PRECIP", "LLUVIA", "CHUBASCO", "AGUA"]):
        add("P")
    if any(x in motivo for x in ["VISIBIL", "NIEBLA", "NEBLINA", "BRUMA"]):
        add("V")
    if any(x in motivo for x in ["NUB", "TECHO", "BASE DE NUBES", "CLOUD BASE", "CIELO"]):
        add("N")
    if any(x in motivo for x in ["FRIO", "BAJA T", "BAJA TEMPERATURA", "CONGEL", "HELADA"]):
        add("TF")
    elif "TEMPERATURA" in motivo or "CALOR" in motivo:
        add("T")
    if any(x in motivo for x in ["NIEVE", "VIENTO BLANCO"]):
        add("VB")

    # 2) Recalcular desde variables del bloque, por si el motivo viene vacío o incompleto.
    th = _op_thresholds(opname or "")
    restrict = th.get("restrict")
    marginal = th.get("marginal")
    wind_ref = max(viento, rachas)
    if restrict is not None and wind_ref >= restrict:
        ...
        add("RV")
    elif wind_ref >= 13 and is_aerial_operation(opname or ""):
        add("RV")
    ...
    if techo is not None and techo > 0:
        # Para SBM el techo es condición operacional crítica: <2500 REST, 2500-3100 MARG, >=3200 FAV.
        if "SALTO BASICO" in opn or opn == "SBM":
            if techo < 3200:
                add("N")
        # Para otras operaciones aéreas, techo/base de nubes bajo también afecta condición visual.
        elif is_aerial_operation(opname or "") and techo < 1000:
            add("N")
    if temp is not None and temp >= 25:
        add("T")
    if temp is not None and temp <= 5:
    ...
    # Oleaje solo aplica a operación costera/embarcación, no a SBM/LC/SLM.
    if ("EMB" in opn or "COST" in opn or "MAR" in opn or "NAUT" in opn) and (oleaje >= 1.5 or "OLEAJE" in motivo or "ESTADO DE MAR" in motivo):
        add("O")

    # 3) Si la celda está marginal/restringida pero no se logró identificar causa, no dejarla vacía.
    est = get_op_state(block, opname or "") if opname else estado_general_bloque(block)
    if not codes and estado_score(est) > 0:
        add("MET")
...
def plain_reason_from_codes(codes):
    ...
    mapping = {
        ...
        "MET": "limitante meteorológica sin detalle",
...
def slide_gantt(prs, data):
    ...
    for r, (op, lab) in enumerate(zip(ops, labels), start=2):
        ...
        for c in range(1, cols):
            ...
            set_cell(table.cell(r, c), codes, 7.2, True, status_color(est))
